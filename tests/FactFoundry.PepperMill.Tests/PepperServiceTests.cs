...
public class PepperServiceTests
{
    ...
    public async Task GetCurrent_GeneratesAndPersists()
    {
        ...
        var result = await service.GetCurrentAsync(Tenant, "site-1");

        Assert.Equal("2026-07", result.Epoch);
        Assert.False(string.IsNullOrEmpty(result.PepperBase64));
        Assert.NotNull(await store.GetAsync(Tenant, "site-1"));
    }
    ...
    public async Task GetCurrent_SameEpoch_ReturnsSamePepper()
    {
        ...
        var first = await service.GetCurrentAsync(Tenant, "site-1");
        var second = await service.GetCurrentAsync(Tenant, "site-1");
    ...
    public async Task GetCurrent_SameSiteId_DifferentTenants_GetDistinctPeppers()
    {
        ...
        var a = await service.GetCurrentAsync("tenant-a", "shared");
        var b = await service.GetCurrentAsync("tenant-b", "shared");
    ...
    public async Task GetCurrent_EpochAdvances_RotatesToNewPepper()
    {
        ...
        var julyPepper = await service.GetCurrentAsync(Tenant, "site-1");
        clock.UtcNow = August;
        var augustPepper = await service.GetCurrentAsync(Tenant, "site-1");
    ...
    public async Task RotateStale_RegeneratesOnlyStaleEpochs()
    {
        ...
        var stored = await store.GetAsync(Tenant, "site-1");
