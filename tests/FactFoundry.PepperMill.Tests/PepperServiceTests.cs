...
public class PepperServiceTests
{
    ...
    public async Task GetCurrent_GeneratesAndPersists()
    {
        ...
        var result = await service.GetCurrentAsync("site-1");

        Assert.Equal("2026-07", result.Epoch);
        Assert.False(string.IsNullOrEmpty(result.PepperBase64));
        Assert.NotNull(await store.GetAsync("site-1"));
    }
    ...
    public async Task GetCurrent_SameEpoch_ReturnsSamePepper()
    {
        ...
        var first = await service.GetCurrentAsync("site-1");
        var second = await service.GetCurrentAsync("site-1");
    ...
    public async Task GetCurrent_EpochAdvances_RotatesToNewPepper()
    {
        ...
        var julyPepper = await service.GetCurrentAsync("site-1");
        clock.UtcNow = August;
        var augustPepper = await service.GetCurrentAsync("site-1");
    ...
    public async Task RotateStale_RegeneratesOnlyStaleEpochs()
    {
        ...
        var stored = await store.GetAsync("site-1");
