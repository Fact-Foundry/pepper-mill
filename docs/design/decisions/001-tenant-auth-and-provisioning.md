...
1. **A fetch is scoped by `(tenantId, siteId)`, and the credential — not the client — decides which site it may read.** Every pepper fetch carries **both** `tenantId` and `siteId` plus a bearer credential (`key2`). PepperMill looks up the credential record for that `(tenantId, siteId)` and requires the presented `key2` to hash to the stored hash; otherwise `403`. So the body's `(tenantId, siteId)` is a *claim* that only succeeds when backed by the matching credential — a caller cannot reach another site's (or tenant's) pepper by changing a field in the body.

2. **Both the pepper and the credential are per *site*, keyed by `(tenantId, siteId)`.** A `siteId` is unique only within its tenant.
